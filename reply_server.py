...
# DEFAULT_ADMIN_PASSWORD removed - admin password now randomly generated by db_manager (package) on first init  # 系统初始化时的默认密码
  # 存储会话token: {token: {'user_id': int, 'username': str, 'timestamp': float}}

  # 下载一次性token: {token_str: {user_id, file_id, exp}}
  # token过期时间：24小时
...
# 扫码登录检查锁 - 防止并发处理同一个session
  ...
  # 记录已处理的session: {session_id: {'processed': bool, 'timestamp': float}}
...
# 账号密码登录会话管理
  # {session_id: {'account_id': str, 'account': str, 'show_browser': bool, 'status': str, 'verification_url': str, 'qr_code_url': str, 'slider_instance': object, 'task': asyncio.Task, 'timestamp': float}}
password_login_locks = defaultdict(lambda: asyncio.Lock())
  # {session_id: {'account_id': str, 'status': str, 'verification_url': str, 'screenshot_path': str, 'slider_instance': object, 'task': asyncio.Task, 'timestamp': float}}
...
# ==================== 自动好评相关API ====================
  ...
  # [{"keyword": str, "reply": str, "item_id": str}]
...
app.include_router(create_cookies_router())
app.include_router(create_orders_chat_router())
app.include_router(create_admin_ops_router())
app.include_router(create_trading_router())
app.include_router(create_account_login_router())
app.include_router(create_keywords_router())
