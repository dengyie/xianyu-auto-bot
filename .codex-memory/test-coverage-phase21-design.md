# Phase 21 Test Coverage Design

## Scope

Broaden delayed-binding terminal discard coverage around the `shipped` resolution branch so `on_order_id_extracted(...)` proves already-consumed shipped messages are discarded instead of rebound onto a newer order.

## Target behavior

1. `on_order_id_extracted()`
   - when a pending terminal system message has `new_status == "shipped"`
   - and another recent order with the same strong match key is already in a shipment-compatible resolved state
...
- Seed:
  ...
  - one queued pending system message with `new_status == "shipped"`
...
1. shipped terminal system message is discarded when another recent order already consumed that outcome
