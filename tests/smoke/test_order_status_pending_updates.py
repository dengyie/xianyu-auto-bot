...
def test_process_all_pending_updates_continues_after_one_bucket_requeues(mocker):
    fake_db = _PendingQueueDB()
    fake_db.orders["queued-order-success"] = {
        "order_id": "queued-order-success",
        "order_status": "processing",
        "pre_refund_status": None,
        "cookie_id": "queued-cookie-success",
    }
    handler = order_status_handler.OrderStatusHandler()
    handler.pending_updates["queued-order-missing"] = [
        {
            "new_status": "pending_ship",
            "cookie_id": "queued-cookie-missing",
            "context": "queued missing order pending ship",
            "timestamp": 1.0,
        }
    ]
    handler.pending_updates["queued-order-success"] = [
        {
            "new_status": "shipped",
            "cookie_id": "queued-cookie-success",
            "context": "queued success order shipped",
            "timestamp": 2.0,
        }
    ]
    ...
    assert processed == 1
    assert fake_db.orders["queued-order-success"]["order_status"] == "shipped"
    assert "queued-order-success" not in handler.pending_updates
