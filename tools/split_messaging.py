...
OUT = ROOT / "xianyu_messaging_mixins.py"
...
def main():
    ...
    missing = [n for n in NOTIFY + PIPELINE + SEND if n not in methods]
    assert not missing, f"missing: {missing}"

    hb = {"asyncio", "base64", "json", "os", "re", "time", "Any", "Dict", "List", "Optional",
          "Tuple", "logger", "_host", "_HostProxy", "_db_package", "_db_host",
          "NotificationMixin", "MessagePipelineMixin", "SendMixin", "XianyuLive",
          "DELIVERY_BATCH_MAX_UNITS", "DELIVERY_BATCH_MAX_CHARS"}

    moved = [methods[n] for n in NOTIFY + PIPELINE + SEND]
    used = {ident for m in moved for ident in (x[4] for x in names_with_pos(m))}
    unknown = used - rs_globals - bi - hb - set(NOTIFY) - set(PIPELINE) - set(SEND) - local_bound(ast.Module(body=moved, type_ignores=[]))
    ...
    def build(names):
        ...
        for n in names:
            ...
            for (ln, col, eln, ecol, ident) in names_with_pos(m):
                ...
                elif ident in rs_globals and ident not in bi and ident not in hb and ident not in set(NOTIFY) and ident not in set(PIPELINE) and ident not in set(SEND):
    ...
    out.extend(t for t in build(NOTIFY))
    out.append("")
    out.append("")
    out.append("class MessagePipelineMixin:")
    out.append('    """消息队列/去重/回复状态机/看门狗/handle_message 热路径。"""')
    out.append("")
    out.extend(t for t in build(PIPELINE))
    out.append("")
    out.append("")
    out.append("class SendMixin:")
    out.append('    """出站消息发送与回复内容生成。"""')
    out.append("")
    out.extend(t for t in build(SEND))
    ...
    dead = [span(methods[n]) for n in NOTIFY + PIPELINE + SEND] + const_dead
    ...
    text = text.replace(anchor,
                        ...
                        "    NotificationMixin,\n"
                        "    SendMixin,\n)\n" + anchor, 1)
    old_cls = "class XianyuLive(OrderMixin, ItemMixin, XianyuAuthRecoveryMixin):"
    assert old_cls in text
    text = text.replace(old_cls,
                        "class XianyuLive(MessagePipelineMixin, SendMixin, NotificationMixin, OrderMixin, ItemMixin, XianyuAuthRecoveryMixin):", 1)
