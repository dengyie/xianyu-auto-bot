...
Playwright 固定为 `1.59.0`。`slidex` 因为是 VCS 依赖，不能参与 pip 的 `--require-hashes`，因此从主锁文件中排除，并以 `--no-deps` 单独安装固定提交 `4a630053ecab6b6b1d4a3aa66cec966f733fe0e0`（0.6.2）；其余依赖仍全部强制哈希校验。
