...
def create_settings_router(ctx) -> APIRouter:
    ...
    def get_system_settings(current_user: Dict[str, Any] = Depends(ctx.require_admin)):
        """获取系统设置（排除敏感信息）"""
        from db_manager import db_manager
        try:
            settings = ctx.db_manager.get_all_system_settings()
            # 移除敏感信息
    ...
    def update_system_setting(key: str, setting_data: ctx.SystemSettingIn, current_user: Dict[str, Any] = Depends(ctx.require_admin)):
        """更新系统设置"""
        from db_manager import db_manager
        try:
            # 禁止直接修改密码哈希
            ...
            value = ctx._validate_system_setting_value(key, setting_data.value)

            if key in ctx.NIGHT_MODE_SYSTEM_SETTING_KEYS and not current_user.get('is_admin'):
                raise HTTPException(status_code=403, detail='仅管理员可修改夜间风控降频设置')

            success = ctx.db_manager.set_system_setting(key, value, setting_data.description)
            if success:
    ...
    def get_registration_status():
        ...
        from db_manager import db_manager
        try:
            enabled_str = ctx.db_manager.get_system_setting('registration_enabled')
    ...
    def get_login_info_status():
        ...
        from db_manager import db_manager
        try:
            enabled_str = ctx.db_manager.get_system_setting('show_default_login_info')
    ...
    def update_registration_settings(setting_data: ctx.RegistrationSettingUpdate, admin_user: Dict[str, Any] = Depends(ctx.require_admin)):
        """更新注册开关设置（仅管理员）"""
        from db_manager import db_manager
        try:
            enabled = setting_data.enabled
            success = ctx.db_manager.set_system_setting(
                'registration_enabled',
                'true' if enabled else 'false',
                '是否开启用户注册'
            )
            if success:
                ctx.log_with_user('info', f"更新注册设置: {'开启' if enabled else '关闭'}", admin_user)
                return {
                    'success': True,
                    'enabled': enabled,
    ...
    def update_login_info_settings(setting_data: ctx.LoginInfoSettingUpdate, admin_user: Dict[str, Any] = Depends(ctx.require_admin)):
        """更新默认登录信息显示设置（仅管理员）"""
        from db_manager import db_manager
        try:
            enabled = setting_data.enabled
            ...
            if success:
                ctx.log_with_user('info', f"更新登录信息显示设置: {'开启' if enabled else '关闭'}", admin_user)
                return {
                    'success': True,
                    'enabled': enabled,
    ...
    def get_login_captcha_settings(admin_user: Dict[str, Any] = Depends(ctx.require_admin)):
        """获取登录验证码设置（仅管理员）"""
        from db_manager import db_manager
        try:
            enabled_str = ctx.db_manager.get_system_setting('login_captcha_enabled')
    ...
    def update_login_captcha_settings(setting_data: ctx.LoginInfoSettingUpdate, admin_user: Dict[str, Any] = Depends(ctx.require_admin)):
        """更新登录验证码设置（仅管理员）"""
        from db_manager import db_manager
        try:
            enabled = setting_data.enabled
            ...
            if success:
                ctx.log_with_user('info', f"更新登录验证码设置: {'开启' if enabled else '关闭'}", admin_user)
                return {
                    'success': True,
                    'enabled': enabled,
    ...
    def get_user_settings(current_user: Dict[str, Any] = Depends(ctx.get_current_user)):
        """获取当前用户的设置"""
        from db_manager import db_manager
        try:
            user_id = current_user['user_id']
            settings = ctx.db_manager.get_user_settings(user_id)
            return settings
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))

    @router.put('/user-settings/{key}')
    def update_user_setting(key: str, setting_data: dict, current_user: Dict[str, Any] = Depends(ctx.get_current_user)):
        """更新用户设置"""
        from db_manager import db_manager
        try:
            user_id = current_user['user_id']
            ...
            ctx.log_with_user('info', f"更新用户设置: {key} = {value}", current_user)

            success = ctx.db_manager.set_user_setting(user_id, key, value, description)
            if success:
                ctx.log_with_user('info', f"用户设置更新成功: {key}", current_user)
                return {'msg': 'setting updated', 'key': key, 'value': value}
            else:
                ctx.log_with_user('error', f"用户设置更新失败: {key}", current_user)
                raise HTTPException(status_code=400, detail='更新失败')
        except HTTPException:
            raise
        except Exception as e:
            ctx.log_with_user('error', f"更新用户设置异常: {key} - {str(e)}", current_user)
            raise HTTPException(status_code=500, detail=str(e))

    @router.get('/user-settings/{key}')
    def get_user_setting(key: str, current_user: Dict[str, Any] = Depends(ctx.get_current_user)):
        """获取用户特定设置"""
        from db_manager import db_manager
        try:
            user_id = current_user['user_id']
            setting = ctx.db_manager.get_user_setting(user_id, key)
            if setting:
                return setting
            else:
    ...
    async def get_sales_data(
        ...
        user_info: Optional[Dict[str, Any]] = Depends(ctx.verify_token)
    ):
        """
        获取销售额数据
        ...
        try:
            ...
            user_cookies = ctx.db_manager.get_all_cookies(current_user_id)
            ...
            query = (
                f"SELECT amount, {ctx.ORDER_SALES_TIME_SQL} AS effective_sales_at, order_status "
                f"FROM orders WHERE cookie_id IN ({placeholders})"
            )
            params = list(cookie_ids)
        
            if start_date:
                utc_start = ctx.local_date_to_utc_start(start_date)
                if not utc_start:
                    raise HTTPException(status_code=400, detail='开始日期格式错误，应为 YYYY-MM-DD')
                query += f" AND {ctx.ORDER_SALES_TIME_SQL} >= ?"
                params.append(utc_start)
            if end_date:
                utc_end_exclusive = ctx.local_date_to_utc_end_exclusive(end_date)
            ...
            # 执行查询
            orders = ctx.db_manager.execute_query(query, params)
            ...
            for order in orders:
                ...
                if not ctx.is_sales_eligible_order_status(order_status):
                    skipped_ineligible_status += 1
                    continue

                amount = ctx.parse_order_amount_value(amount_str)
                if amount is None:
                    skipped_invalid_amount += 1
                    continue

                local_date = ctx.utc_timestamp_to_local_date_string(effective_sales_at)
                if not local_date:
                    continue
    ...
    async def get_sales_summary(
        ...
        try:
            ...
            # 单次查询获取所有数据，减少数据库访问
            placeholders = ','.join(['?'] * len(cookie_ids))
            month_start_utc = ctx.local_date_to_utc_start(month_start_str)
            query = (
                f"SELECT amount, {ctx.ORDER_SALES_TIME_SQL} AS effective_sales_at, order_status "
                f"FROM orders WHERE {ctx.ORDER_SALES_TIME_SQL} >= ? AND cookie_id IN ({placeholders})"
            )
            all_orders = ctx.db_manager.execute_query(query, [month_start_utc] + cookie_ids)
            ...
            for order in all_orders:
                ...
                local_effective_sales_at = ctx.utc_timestamp_to_local_datetime(effective_sales_at)
