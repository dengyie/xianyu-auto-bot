...
def _invoke_slider_sync(slider: Any, url: str, **kwargs: Any) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """兼容 slider.run / slider.solve 同步入口。"""
    if hasattr(slider, "run") and callable(getattr(slider, "run")):
        return slider.run(url, **kwargs)
    if hasattr(slider, "solve") and callable(getattr(slider, "solve")):
        result = slider.solve(url, **kwargs)
        # 极少数同步 solve 直接返回；异步 solve 不应走到这里
        if hasattr(result, "__await__"):
            raise TypeError("slider.solve is async; use run_slider_async_* helpers")
        return result
...
def run_slider_strict(
    ...
    **kwargs: Any,
    ...
    """调用同步 slider.run/solve，并进行严格 x5sec 判定。"""
    success, cookies = _invoke_slider_sync(slider, url, **kwargs)
