...
def _get_db():
    # 延迟绑定，避免 import 时固化单例导致测试/热替换失效
...
async def rate_order_once(
    ...
    *,
    ...
    if not order_info:
        message = "订单不存在"
        db.add_scheduled_rate_log(batch_id, cookie_id, order_id=order_id, status="skipped", message=message)
    ...
    if not comment:
        template = db.get_active_comment_template(cookie_id)
    ...
    cookie_string = db.get_cookie(cookie_id)
    ...
    if result.get("success"):
        db.mark_order_rated(order_id, True)
    else:
        db.mark_order_rated(order_id, False, message)
...
async def run_auto_rate_batch(
    *,
    ...
    for cookie_id in list(all_cookies.keys()):
        try:
            if not db.get_auto_comment(cookie_id):
                continue
            stats["accounts"] += 1
            template = db.get_active_comment_template(cookie_id)
            if not template or not str(template.get("content") or "").strip():
            ...
            orders = db.get_pending_auto_comment_orders(
                cookie_id,
                limit=batch_limit,
                days=lookback_days,
